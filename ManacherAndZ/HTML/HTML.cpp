...
// Function to generate an HTML file that highlights occurrences of the longest palindromic substring
void generateHTML(const string& text, const string& palindrome, const vector<int>& positions) {
    ofstream htmlFile("highlighted_palindrome.html");
    if (!htmlFile.is_open()) {
    ...
    for (int i = 0; i < text.length(); ++i) {
        if (pos < positions.size() && i == positions[pos]) {
            // Highlight the palindrome in red
            htmlFile << "<span style='color:red;'>" << text.substr(i, palindrome.length()) << "</span>";
            i += palindrome.length() - 1; // Skip ahead by the length of the palindrome
            pos++;
        } else {
            // Write normal text
            htmlFile << text[i];
    ...
    htmlFile.close();
    cout << "HTML file created successfully: highlighted_palindrome.html" << endl;
}
...
int main() {
    ...
    // Generate an HTML file highlighting the palindromic substring in the text
    generateHTML(text, longestPalindrome, occurrences);
